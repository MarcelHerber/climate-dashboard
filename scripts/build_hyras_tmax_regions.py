...
USER_AGENT = "climate-dashboard-hyras-tmax-regions/1.0 (+GitHub Actions; DWD Open Data)"
...
def latest_daily_files(text: str) -> dict[int, str]:
    pat = re.compile(
        r'href="(?P<filename>tasmax_hyras_1_(?P<year>\d{4})_'
...
def pick_var(ds: xr.Dataset) -> xr.DataArray:
    for name in ("tasmax", "tmax", "air_temperature_max", "temperature"):
        if name in ds.data_vars and ds[name].ndim >= 2:
            return ds[name]
    for name, da in ds.data_vars.items():
        if da.ndim >= 2:
            print(f"Hinweis: verwende Tmax-Variable {name}", flush=True)
            return da
    raise RuntimeError(f"Keine HYRAS-Tmax-Variable gefunden: {list(ds.data_vars)}")
...
def spatial_dims(da: xr.DataArray) -> tuple[str, str]:
    ...
    if len(dims) != 2:
        raise RuntimeError(f"Unerwartete HYRAS-Tmax-Dimensionen: {da.dims}")
...
def prepare_da(ds: xr.Dataset) -> tuple[xr.DataArray, str, np.ndarray, np.ndarray]:
    ...
    if x.ndim != 1 or y.ndim != 1:
        raise RuntimeError("HYRAS-Tmax x/y-Koordinaten sind nicht eindimensional.")
    return da, td, x, y
...
def extract_region_daily(
    ...
    if expected_x is not None and (x.shape != expected_x.shape or not np.allclose(x, expected_x)):
        raise RuntimeError("Historisches HYRAS-Tmax-x-Gitter weicht vom aktuellen Gitter ab.")
    if expected_y is not None and (y.shape != expected_y.shape or not np.allclose(y, expected_y)):
        raise RuntimeError("Historisches HYRAS-Tmax-y-Gitter weicht vom aktuellen Gitter ab.")
...
def climate_payload(
    ...
    return {
        ...
        "note": (
            "Tägliches HYRAS-Tmax-Gebietsmittel: zuerst räumliches Mittel der "
            "Tagesmaxima je Gebiet und Tag, danach Kalendertagsmittel 1991–2020."
        ),
    }
...
def main() -> int:
    ...
    ap.add_argument("--work", default="/tmp/hyras-tmax-work")
    ap.add_argument("--force-history-summary", action="store_true")
    args = ap.parse_args()

    data_root = Path(args.data_root)
    work = Path(args.work)
    out_root = data_root / "tmax"
    ...
    if not files:
        raise RuntimeError("Keine HYRAS-Tmax-Tagesdateien gefunden.")
    ...
    if not current_dates:
        raise RuntimeError("Aktuelle HYRAS-Tmax-Datei enthält keine Tage.")
    ...
    if (
        ...
        print(
            f"Verwende vorhandene Tmax-Klimakurve {REFERENCE_START}–{REFERENCE_END} "
            f"und Tagesrekorde {RECORD_FIRST_YEAR}–{record_last_year}.",
            flush=True,
        )
    else:
        print(
            f"Baue Tmax-Klimakurve {REFERENCE_START}–{REFERENCE_END} und "
        ...
        if missing:
            raise RuntimeError(
                "HYRAS-Tmax-Tagesdateien fehlen für: " + ", ".join(map(str, missing))
            )
        ...
        for year in range(RECORD_FIRST_YEAR, record_last_year + 1):
            ...
            print(f"Tmax Historie {year}: {filename}", flush=True)
        ...
        records_payload = {
            ...
            "note": (
                ...
                "HYRAS-Tmax-Gebietsmittels je Kalendertag."
            ),
        }
    ...
    index = {
        ...
        "method_note": (
            "HYRAS-DE-TASMAX: tägliches räumliches Mittel aller 1-km-Gitterzellen "
    ...
    if len(climate["regions"].get("Deutschland", {})) < 365:
        raise RuntimeError("Tmax-Klimakurve Deutschland ist unvollständig.")
    if len(records["regions"].get("Deutschland", {})) < 365:
        raise RuntimeError("Tmax-Rekordkalender Deutschland ist unvollständig.")

    print("HYRAS Tmax Datenbasis fertig.", flush=True)
