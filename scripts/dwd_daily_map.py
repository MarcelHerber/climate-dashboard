...
def write_daily_map(output_dir:Path,*,stations,baseline,current,current_year:int,station_listing_text:str="",generated_at:str|None=None):
    ...
    ids=sorted((sid for sid in current if sid in stations and (current[sid].get("TMAX") or current[sid].get("TMIN"))),key=lambda sid:(str(getattr(stations[sid],"name","")).casefold(),sid))
    ...
    for iso in dates:
        ...
        for sid in ids:
            ...
            tx,txd=_current(cur,"TMAX",mmdd); tn,tnd=_current(cur,"TMIN",mmdd)
            if tx is None and tn is None:continue
            if any(v!=dateint for v in (txd,tnd) if v is not None):continue
            ...
            if txa is not None or tna is not None:anoms+=1
            ...
            rows.append([pos[sid],tx,tn,txa,tna,txs,tns,_record(hist,"TMAX",mmdd),_record(hist,"TMIN",mmdd),txc,tnc])
...
def self_test():
    ...
    hist={"DWD:00001":{"TMAX":{"cal":{"08-28":(300,20200828,1)},"clim_1991_2020":{"08-28":[4000,20]}},"TMIN":{"cal":{"08-28":(50,19930828,1)},"clim_1991_2020":{"08-28":[2000,20]}}}}
    cur={"DWD:00001":{"TMAX":{"08-28":(310,20260828)},"TMIN":{"08-28":(90,20260828)}}}
