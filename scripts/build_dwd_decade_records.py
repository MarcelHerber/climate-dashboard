...
class DecadeAccumulator:
    def __init__(self) -> None:
        self.leaders: dict[str, dict[str, dict[str, TopK]]] = defaultdict(
    ...
    def add(self, observation: Observation) -> None:
        ...
        for metric in METRIC_CONFIG:
            if metric not in observation.values:
                continue
            entry = entry_for(observation, observation.values[metric])
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="DWD-Dekadenrekorde für Deutschland und alle Bundesländer bauen."
    )
    parser.add_argument(
        "--output",
        default="data/dwd_decade_records.json",
        help="Ziel-JSON",
    )
    ...
    print("=== DWD DEKADENREKORDE · STUFE 1 ===", flush=True)
    print("Parameter: Tmax hoch + Tmin tief", flush=True)
    print("Gebiete: Deutschland + 16 Bundesländer", flush=True)
    print("Fenster: 36 meteorologische Dekaden", flush=True)

    metadata_bytes = download(METADATA_URL, timeout=90)
    ...
    historical_files = list_station_files(
        HISTORICAL_URL, HISTORICAL_PATTERN, minimum=500
    )
    recent_files = list_station_files(
        RECENT_URL, RECENT_PATTERN, minimum=300
    )
    ...
    payload = {
        ...
        "method_note": (
            ...
            "3.=21.–Monatsende. Rekorde werden aus täglichen DWD-KL-Werten "
            "TXK (Tmax) und TNK (Tmin) gebildet. Deutschland umfasst alle "
            "verfügbaren deutschen Stationen; Bundesländer folgen den "
            "DWD-Stationsmetadaten zum jeweiligen Beobachtungstag."
        ),
    }
    ...
    if set(payload["metrics"]) != {"txk_high", "tnk_low"}:
