...
from build_hyras_tmin_regions import DAILY_BASE, USER_AGENT, download, latest_daily_files, pick_var, prepare_da, to_celsius
...
def latest_clim_filename(month,text=None):
    ...
    pat=re.compile(rf'href="(?P<filename>tasmin_hyras_1_{REFERENCE_START}_{REFERENCE_END}_v(?P<major>\d+)-(?P<minor>\d+)_de_{abbr}\.nc)"')
    m=[(int(x.group("major")),int(x.group("minor")),x.group("filename")) for x in pat.finditer(text)]
    if not m: raise RuntimeError(f"Keine Tmin-Klimadatei für {abbr} gefunden.")
    return sorted(m)[-1][2]

def static_2d(ds):
    da=to_celsius(pick_var(ds)).squeeze(drop=True)
    while da.ndim>2: da=da.isel({da.dims[0]:0})
    if da.ndim!=2: raise RuntimeError(f"Keine statische 2D-Tmin-Klimatologie: {da.dims}")
...
def reference_for_months(months,year,work,text,ex,ey):
    ...
    for m in months:
        ...
        if x.shape!=ex.shape or y.shape!=ey.shape or not np.allclose(x,ex) or not np.allclose(y,ey): raise RuntimeError(f"Tmin-Klimaraster {fn} passt nicht zum aktuellen Gitter.")
        days=calendar.monthrange(year,m)[1]; weighted=a*days if weighted is None else weighted+a*days; total+=days
    if weighted is None or total<=0: raise RuntimeError("Keine Tmin-Referenzmonate vorhanden.")
    return weighted/float(total)

def anomaly_style():
    lev=np.asarray(LEVELS,float); mids=(lev[:-1]+lev[1:])/2; bounds=np.r_[[lev[0]-.5],mids,[lev[-1]+.5]]; cmap=ListedColormap(COLORS,name="hyras_tmin_anomaly_final"); return cmap,BoundaryNorm(bounds,cmap.N,clip=True)
...
def render_map(a,boundary,output,title,subtitle,mode):
    ...
    if anom:
        ...
        cmap=LinearSegmentedColormap.from_list("hyras_tmin_absolute",["#4a1486","#313695","#4575b4","#74add1","#abd9e9","#e0f3f8","#ffffbf","#fee090","#fdae61","#f46d43","#d73027"]); ax=fig.add_axes([.055,.105,.89,.80]); im=ax.imshow(a,cmap=cmap,vmin=-25.0,vmax=25.0,interpolation="nearest")
    ax.set_axis_off()
    ...
    fig.text(.055,.012 if anom else .018,"Quelle: Deutscher Wetterdienst · HYRAS-DE-TASMIN · Referenz 1991–2020",fontsize=7,color="#555555")
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--data-root",default="/tmp/hyras-data"); ap.add_argument("--work",default="/tmp/hyras-tmin-work"); args=ap.parse_args()
    root=Path(args.data_root); work=Path(args.work); troot=root/"tmin"; regions=troot/"regions"; ip=regions/"index.json"
    if not ip.exists(): raise RuntimeError("Tmin-Regionsindex fehlt.")
    ...
    if not fn: raise RuntimeError(f"Keine aktuelle Tmin-Datei für {year} gefunden.")
    nc=work/fn
