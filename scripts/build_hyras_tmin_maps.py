...
from build_hyras_tmin_regions import (
    DAILY_BASE,
    USER_AGENT,
    download,
    latest_daily_files,
...
def latest_clim_filename(month: int, text: str | None = None) -> str:
    ...
    pattern = re.compile(
        rf'href="(?P<filename>tasmin_hyras_1_{REFERENCE_START}_{REFERENCE_END}_'
    ...
    if not matches:
        raise RuntimeError(f"Keine Tmin-Klimadatei für {abbr} gefunden.")
    matches.sort()
    return matches[-1][2]
...
def static_2d(ds: xr.Dataset) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if da.ndim != 2:
        raise RuntimeError(f"Keine statische 2D-Tmin-Klimatologie: {da.dims}")
...
def period_current_mean(
    ...
    if selected.sizes.get(td, 0) == 0:
        raise RuntimeError(f"Keine Tmin-Tage für {start_date} bis {end_date}.")
...
def reference_for_months(
    ...
    for month in months:
        ...
        if (
            ...
            raise RuntimeError(f"Tmin-Klimaraster {filename} passt nicht zum aktuellen Gitter.")
    ...
    if weighted is None or total_days <= 0:
        raise RuntimeError("Keine Tmin-Referenzmonate vorhanden.")
...
def get_colormap(mode: str):
    if mode == "anomaly":
        ...
        return LinearSegmentedColormap.from_list("hyras_tmin_anomaly", colors), -6.0, 6.0, "K"

    colors = [
        "#4a1486","#313695","#4575b4","#74add1","#abd9e9",
        "#e0f3f8","#ffffbf","#fee090","#fdae61","#f46d43","#d73027",
    ]
    return LinearSegmentedColormap.from_list("hyras_tmin_absolute", colors), -25.0, 25.0, "°C"
...
def render_map(
    ...
    fig.text(
        0.055,
        0.018,
        "Quelle: Deutscher Wetterdienst · HYRAS-DE-TASMIN · Referenz 1991–2020",
        fontsize=7,
        color="#555555",
    )
...
def main() -> int:
    ...
    work = Path(args.work)
    troot = data_root / "tmin"
    regions_dir = troot / "regions"

    index_path = regions_dir / "index.json"
    if not index_path.exists():
        raise RuntimeError("Tmin-Regionsindex fehlt.")
    ...
    if not current_filename:
        raise RuntimeError(f"Keine aktuelle Tmin-Datei für {year} gefunden.")
    ...
    for period in index.get("periods", []):
        ...
        render_map(
            ...
            troot / absolute_rel,
            f"HYRAS Tmin · {label}",
            f"2-m-Tagesminimum · {start_date} bis {end_date}",
            "absolute",
        )
        ...
        result[key] = item
        print(
            f"Tmin Downloadkarte {key}: absolut"
    ...
    print("Tmin-Kartendownloads fertig.", flush=True)
