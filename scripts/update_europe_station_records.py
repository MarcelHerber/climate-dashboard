...
def parse_current_ghcn_year(year: int, stations: Dict[str, StationMeta]) -> dict:
    ...
    for meta in stations.values():
        country_counts[meta.country_code] = country_counts.get(meta.country_code, 0) + 1
    spain_candidates = country_counts.get("SP", 0)
    austria_candidates = country_counts.get("AU", 0)
    log(
        f"Lade laufendes GHCN-Jahr {year} für {len(stations):,} freigegebene Europa-Stationen "
        f"(Spanien {spain_candidates:,}, Österreich {austria_candidates:,}): {url}"
    )
    ...
    spain_current = sum(1 for sid in current if sid in stations and stations[sid].country_code == "SP")
    austria_current = sum(1 for sid in current if sid in stations and stations[sid].country_code == "AU")
    log(
        f"GHCN {year}: {len(current):,} freigegebene Stationen mit TMAX/TMIN "
        f"(darunter Spanien {spain_current:,}, Österreich {austria_current:,})."
    )
    return current
