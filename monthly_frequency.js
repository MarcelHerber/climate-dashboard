/* MONTHLY_FREQUENCY_DISTRIBUTION_V2 */
(function(){
  "use strict";
  ...
  function buildDistribution(valuesA,valuesB,mode){
    ...
    const count=Math.round((end-start)/BIN_STEP)+1;const centers=Array.from({length:count},(_,index)=>Number((start+index*BIN_STEP).toFixed(6)));
  ...
  function buildIndividualBars(rowsA,rowsB,mode){
    const cleanA=rowsA.filter(row=>Number.isFinite(row?.value));const cleanB=rowsB.filter(row=>Number.isFinite(row?.value));const combined=[];
    ...
    combined.forEach(item=>{const key=Number(item.row.value).toFixed(6);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(item);});
    const weightA=mode==="percent"&&cleanA.length?100/cleanA.length:1;const weightB=mode==="percent"&&cleanB.length?100/cleanB.length:1;
    const pointsA=[],pointsB=[];
    groups.forEach(group=>{
      group.sort((a,b)=>a.period.localeCompare(b.period)||Number(a.row.year)-Number(b.row.year));
      const value=Number(group[0].row.value);const n=group.length;const maxSpread=0.08;
      group.forEach((item,index)=>{
        const offset=n===1?0:-maxSpread/2+maxSpread*index/(n-1);
        const point={x:Number((value+offset).toFixed(6)),y:item.period==="A"?weightA:weightB,value,year:item.row.year};
  ...
  function empiricalPercentile(values,value){if(!values.length||!Number.isFinite(value))return null;const below=values.filter(item=>item<value).length;const equal=values.filter(item=>item===value).length;return 100*(below+0.5*equal)/values.length;}

  globalThis.__monthlyFrequencyTestApi={buildDistribution,buildIndividualBars,empiricalPercentile,average,sampleStdDev};
  if(typeof document==="undefined"||typeof Chart==="undefined")return;

  let chart=null;let mode="classic";let previousParam=null;let dataWaitTimer=null;
  ...
  function render(){
    ...
    const area=document.getElementById("monthlyAreaSelect")?.value||"Deutschland",selection=document.getElementById("monthSelect")?.value||"07";
    ...
    const rowsA=filterRange(series,rangeA),rowsB=filterRange(series,rangeB),valuesA=rowsA.map(item=>item.value),valuesB=rowsB.map(item=>item.value);
    const requested=document.getElementById("monthlyFrequencyScale")?.value||"auto",yMode=requested==="auto"?(valuesA.length===valuesB.length?"count":"percent"):requested,display=document.getElementById("monthlyFrequencyDisplay")?.value||"individual";
    const prepared=display==="individual"?buildIndividualBars(rowsA,rowsB,yMode):buildDistribution(valuesA,valuesB,yMode);
    ...
    const barA={type:"bar",label:rangeA.label,data:prepared.pointsA,parsing:false,backgroundColor:"rgba(43,111,142,.45)",borderColor:BLUE,borderWidth:1,barThickness:display==="individual"?5:20,order:3};
    const barB={type:"bar",label:rangeB.label,data:prepared.pointsB,parsing:false,backgroundColor:"rgba(196,59,47,.38)",borderColor:RED,borderWidth:1,barThickness:display==="individual"?5:20,order:3};
    const curveA={type:"line",label:`Normalverteilung ${rangeA.label}`,data:prepared.normalA,parsing:false,borderColor:BLUE,borderWidth:2.4,pointRadius:0,tension:.28,order:1};
    const curveB={type:"line",label:`Normalverteilung ${rangeB.label}`,data:prepared.normalB,parsing:false,borderColor:RED,borderWidth:2.4,pointRadius:0,tension:.28,order:1};
  ...
  function init(){
    ...
    document.getElementById("monthlyFrequencyScale").addEventListener("change",render);document.getElementById("monthlyFrequencyDisplay").addEventListener("change",render);document.getElementById("monthlyAreaSelect")?.addEventListener("change",()=>{if(mode==="frequency")render();});document.getElementById("monthSelect")?.addEventListener("change",()=>{if(mode==="frequency")render();});
